          ...
          <div v-for="(item,index) in orderList" :key="index">
            ...
            <div class="pr" style="position:relative;">
              <div class="cart" :key="index">
                <div class="cart-l" :class="{bt:index>0}">
                  <div class="car-l-l">
                    <div class="img-box"><a><img  v-lazy="require(`../../static/${item.productimg}`)" alt=""></a></div>
...
import paySuc from '@/components/shop/paySuc'
import $ from 'jquery'
  ...
  export default {
    data () {
      return {
        ...
        selectType:0,
        payC:false,
        payType:require('../../static/images/payali.png'),
        payTitle:'打开支付宝',
        totalPrice:'',
        orId:'',
        payList:[
            {
              id:1,
              imgUrl:require('../../static/images/alipay@2x.png')
            },
            {
              id:2,
              imgUrl:require('../../static/images/weixinpay@2x.png')
            },
            {
              id:3,
              imgUrl:require('../../static/images/qqpay.png')
            }]
      }
    },
    mounted() {
    ...
    methods: {
      getOrderData() {
          API.getOrderData({userid:JSON.parse(localStorage.getItem('user')).userId}).then((res) => {
      ...
      select(type) {
        ...
        if(type == 0) {
          this.payType = require('../../static/images/payali.png');
          this.payTitle = '打开支付宝';
        } else {
          this.payType = require('../../static/images/weixin.png');
          this.payTitle = '打开微信';
        }
      },
      yesPay() {
      ...
      pay() {
        var param = {
          ...
          API.updateOrder(param).then((res) => {
          if(res.status == '1') {
            setTimeout(()=>{
              this.mdShow = false;//隐藏名字
              this.$router.push({
