          ...
          <div class="cart-item order-item">
            ...
            <ul class="cart-item-list">
              <li v-for="item in cartList" :key="item.id">
                <div class="cart-tab-1"  v-if="item.checked=='1'">
                  <div class="cart-item-pic">
                    <img :src="require(`../../static/${item.productimg}`)" alt="XX">
                  </div>
      ...
      methods: {
        ...
        yesPay() {
         ...
         pay() {
          ...
          API.payOrder(param).then((res) => {
              console.log("进入支付成功界面");
            if(res.status == '1') {
                let Base64 = require('js-base64').Base64;
              setTimeout(()=>{
                this.mdShow = false;
                this.$router.push({
                  path:`/index/orderinfo?m=${Base64.encode(res.result)}&n=${Base64.encode(this.totalPrice)}`
              });
              }, 6000);
            }
          });
